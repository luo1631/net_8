...
LOG_DIR = os.path.join(SCRIPT_DIR, "runs", "vgg11_cifar10")
...
def main():
    ...
    # -------------------- 打印配置信息 --------------------
    print("=" * 95)
    print("VGG-11 Training on CIFAR-10")
    print("=" * 95)
    ...
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.RandomErasing(p=0.5, scale=(0.02, 0.1), ratio=(0.3, 3.3), value='random'),  #随机擦除,感受野对于cifar10来说有点大了，以此减少过拟合
    ...
    optimizer = optim.SGD([
        {'params': weight_params, 'lr': LEARNING_RATE, 'weight_decay': WEIGHT_DECAY},
        {'params': bias_params,   'lr': LEARNING_RATE * 0.5, 'weight_decay': 5e-5 }
    ], momentum=MOMENTUM)
    ...
    #余弦退火+重启
    #scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer,T_0=20,T_mult=1,eta_min=5e-5)
    ...
    final_path = os.path.join(SAVE_DIR, f"vgg11_{timestamp}_acc{best_acc:.1f}.pth")
